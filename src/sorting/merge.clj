...
(defn divide
  "Divide a sequence by finding the midway positon and retun the two halves."
  [xs]
  (let [cnt (count xs)
        idx (quot cnt 2)]
...
(defn merge
  "Assuming a and b are sorted vectors merge them so that the result are the contents of the two
  are combined into a single sorted array"
  [xs1 xs2]
  ;; (println xs1 xs2)
  (let [a (first xs1)
        b (first xs2)]
...
(defn merge-sort
  "Merge sort by recursively dividing the sequence until you have a pair to return sorted. Merge the returned results
  by combining two sequences so the two sorted sequences are merged into a single sorted sequence."
  [xs]
  (if (seq xs)
    (cond
      (< (count xs) 2) xs
